/**
 * Bundled copy of @tmustier/pi-queue-steer (MIT, author Thomas
 * Mustier, https://github.com/tmustier/pi-queue-steer): visible steering
 * and follow-up queues with inline editing.
 *
 * Changes vs upstream:
 * - exposed as a register function gated by a live `isEnabled` getter so
 *   the /claude-coder settings toggle takes effect without a Pi reload;
...
export function registerBundledQueueSteer(pi: ExtensionAPI, isEnabled: () => boolean): void {
	...
	const renderQueue = (ctx: ExtensionContext): void => {
		...
		// live isEnabled() gate so the /claude-coder toggle works without a Pi reload.
		if (!isEnabled()) {
	...
	const installEditor = (ctx: ExtensionContext): void => {
		...
		const factory = ((tui, theme, keybindings) => {
			...
			editor.handleInput = (data: string): void => {
				...
				// Insert directly: forwarding the original bytes lets an underlying
				// editor reinterpret them as app.message.followUp.
				if (
					...
					const newlineEditor = editor as typeof editor & { addNewLine?: () => void };
					if (typeof newlineEditor.addNewLine === "function") {
						newlineEditor.addNewLine();
						return;
					}
					handleInput(data);
					return;
				...
				if (
					...
					// A typed draft owns Esc (double-esc-clear hint/clear); only an
