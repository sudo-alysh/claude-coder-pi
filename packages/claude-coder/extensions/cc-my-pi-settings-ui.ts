/**
 * Interactive /claude-coder settings overlay with live ASCII previews.
 */
import {
	...
	},
	{
		...
		describe: (s) =>
			s.spinnerVerbColor === "borderAccent"
				? "Verb text (Cooking…) uses borderAccent (default) — more keys/hex: /claude-coder spinner"
				: `Verb text (Cooking…) uses ${s.spinnerVerbColor} — more keys/hex: /claude-coder spinner`,
	},
	{
...
export async function openCcToolsSettingsPanel(
	...
	if (!ctx?.hasUI) {
		ctx?.ui?.notify?.("/claude-coder UI requires TUI mode", "error");
		return;
	}
				...
				render(width: number) {
					...
					const header = [
						safeFg(
							theme,
							"accent",
							theme.bold?.("Claude Coder settings") ?? "Claude Coder settings",
						) +
							safeFg(
								theme,
								"muted",
 ...
 * (e.g. a `#d77757` set via `/claude-coder spinner verb`). Cycling then starts from
...
export async function openCcToolsSetupWizard(
	...
	if (!ctx?.hasUI) {
		ctx?.ui?.notify?.("/claude-coder setup requires TUI mode", "error");
		return "skip-once";
	}
	...
	await ctx.ui.custom(
		(
			...
			const renderIntro = (width: number): string[] => {
				const titleText = "Claude Coder setup";
			...
			const renderCompanions = (width: number): string[] => {
				const titleText = "Claude Coder setup — optional extensions";
				const lines: string[] = [
					safeFg(theme, "accent", theme.bold?.(titleText) ?? titleText),
					safeFg(
						theme,
			...
			const renderStep = (width: number): string[] => {
				const def = steps[stepIndex]!;
				const titleText = `Claude Coder setup — step ${stepIndex + 1}/${steps.length}: ${def.label}`;
				const title = safeFg(
					theme,
					"accent",
