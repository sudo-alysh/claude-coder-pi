...
// Short TTL so /claude-coder spinner changes are picked up within ~1s without
...
// Cross-extension bust signal: /claude-coder spinner in index.ts bumps this counter
...
export default function (pi: ExtensionAPI) {
	...
	function syncWorkingMessage(force = false): void {
		...
		// Re-derive colors on every tick so /claude-coder spinner verb/status changes
	...
	function getWorkingMessageIntervalMs(): number {
		const elapsed = Date.now() - (agentStartTime || turnStartTime);
		const tokenCount = Math.max(0, Math.round(responseLength / 4));
		// Keep ticking once per second even when idle so /claude-coder spinner changes
