...
function paintThinkingLevel(level: unknown, text: string, muted: (s: string) => string): string {
	const rgb = THINKING_COLORS[String(level)];
	return rgb ? `\x1b[38;2;${rgb}m${text}\x1b[39m` : muted(text);
}
...
class PiStartupHeader implements Component {
	...
	private rightLines(muted: (s: string) => string, dim: (s: string) => string, accent: (s: string) => string, bold: (s: string) => string, rightWidth: number): string[] {
		...
		return [
			accent(bold("Quick controls")),
			muted("Ctrl+O expands tool output · Esc interrupts"),
			divider,
			accent(bold("Loaded")),
			countsLine,
			aggregateLine,
	...
	render(width: number): string[] {
		...
		const effort = formatThinkingLabel(thinkingLevel);
		const cwd = formatCwd(this.ctx.cwd);

		// Left column order: mascot → model·effort → cwd.
		const leftLines = [
			...piMascotFrame(this.frame, { accent: paint, muted }).map((line) => center(line, leftWidth)),
			center(
				`${muted(`${model} · `)}${paintThinkingLevel(thinkingLevel, `${effort} effort`, muted)}`,
				leftWidth,
			),
		...
		const lines = [borderLine("╭", paint(`Claude Coder v${VERSION}`), "╮", width, paint)];
