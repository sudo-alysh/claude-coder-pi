...
// Types only — settings UI module (~1k LOC + panel stack) loads on first /claude-coder open.
import type {
	BranchPreset,
	...
	SetupWizardOutcome,
	ToolStyle,
} from "./claude-coder-settings-ui.js";
...
type SettingsUiModule = typeof import("./claude-coder-settings-ui.js");
...
function loadSettingsUi(): Promise<SettingsUiModule> {
	return (settingsUiModule ??= import("./claude-coder-settings-ui.js"));
}
...
// /claude-coder spinner edits take effect on the next 250ms spinner tick instead of
...
// theme / /claude-coder branch / /claude-coder theme rebinds; the mode is included because subagent
...
function patchCustomMessageRender(): void {
	...
	proto.render = function patchedCustomMessageRender(width: number) {
		...
		// can change via /claude-coder or by editing settings.json. Re-sync before the
...
const PROMPT_GLYPH_FEATURE = "cc-prompt-glyph-spin-v2";
const EDITOR_FEATURES_SYMBOL = Symbol.for("@tmustier/pi-editor-features");
const BORDER_STRIP_GLYPHS: Record<string, string> = {
	"─": "━",
	"│": "┃",
	"╭": "┏",
	"╮": "┓",
...
const BORDER_STRIP_SPEED_MS: Record<string, number> = {
	minimal: 260,
	low: 190,
	medium: 140,
	high: 100,
...
let borderStripPhase = 0;
let borderStripTimer: NodeJS.Timeout | undefined;
let borderStripEditor: any;
let borderStripTui: any;

function replaceVisibleBorderCell(line: string, target: number): string {
	let column = 0;
	for (let index = 0; index < line.length; ) {
		...
		if (column === target && BORDER_STRIP_GLYPHS[character]) {
			return `${line.slice(0, index)}${BORDER_STRIP_GLYPHS[character]}${line.slice(index + character.length)}`;
		}
...
function animateEditorBorder(lines: string[], width: number): string[] {
	const height = lines.length;
	if (height < 3 || width < 4) return lines;
	const vertical = height - 2;
	const perimeter = width * 2 + vertical * 2;
	for (let strip = 0; strip < perimeter; strip += 14) {
		for (let offset = 0; offset < 3; offset++) {
			let position = (borderStripPhase + strip + offset) % perimeter;
			let x: number;
			let y: number;
			if (position < width) {
				x = position;
				y = 0;
			} else if ((position -= width) < vertical) {
				x = width - 1;
				y = position + 1;
			} else if ((position -= vertical) < width) {
				x = width - 1 - position;
				y = height - 1;
			} else {
				position -= width;
			...
			lines[y] = replaceVisibleBorderCell(lines[y] ?? "", x);
		}
	}
	return lines;
}
...
function registerPromptGlyphWrapper(pi: ExtensionAPI): void {
	const install = (ctx: any): void => {
		...
		const factory = ((tui: any, theme: any, keybindings: any) => {
			...
			editor.render = (width: number): string[] => {
				...
				if (alreadyDecorated) return animateEditorBorder(lines, width);
				...
				return animateEditorBorder(lines, width);
			};
			return editor;
		}) as any;
		factory[EDITOR_FEATURES_SYMBOL] = new Set([
	...
	const startBorderAnimation = (ctx: any): void => {
		if (borderStripTimer) clearTimeout(borderStripTimer);
		const tick = () => {
			const level = pi.getThinkingLevel();
			if (level !== "off") {
				borderStripPhase++;
				borderStripEditor?.invalidate?.();
				borderStripTui?.requestRender?.();
				ctx.ui.requestRender?.();
			}
			const speed = BORDER_STRIP_SPEED_MS[level] ?? 300;
			borderStripTimer = setTimeout(tick, speed);
			borderStripTimer.unref?.();
		};
		tick();
	};
	pi.on("session_start", (_event, ctx) => {
		installLate(ctx);
		startBorderAnimation(ctx);
	});
	pi.on("agent_start", (_event, ctx) => installLate(ctx));
	pi.on("session_shutdown", () => {
		if (borderStripTimer) clearTimeout(borderStripTimer);
		borderStripTimer = undefined;
	});
}
...
function refreshAllToolBranchVisuals(ctx: any): void {
	...
	bumpToolBranchVisualEpoch(); // always bust ToolText + container caches after /claude-coder branch
...
export default async function (pi: ExtensionAPI) {
	...
	// /claude-coder command — control tool chrome, grouping, detail, and settings UI.
	...
	const applyCcToolsUiSetting = (id: string, value: string, ctx: any): void => {
		...
		switch (id) {
			...
			case "spinnerStatusColor": {
				if (!value) return;
				// Default keys are stored as "unset" so /claude-coder spinner reset stays equivalent.
	...
	const notifyToolStatus = (ctx: any): void => {
		...
		ctx.ui.notify(
			[
				...
				branchLine,
				`  /claude-coder branch <0-255> | theme | fixed | reset`,
				`Diff preview: ${diffCollapsedLimit() ?? "stock (24 write / 32 edit)"} collapsed lines`,
				`  /claude-coder diff <1-150> | stock | status`,
			].join("\n"),
			"info",
		);
	};
	pi.registerCommand("claude-coder", {
		description:
			"Open Claude Coder settings (or setup/style/group/branch/diff/theme/spinner subcommands)",
		...
		async handler(args, ctx) {
			...
			const sub = parts[0] ?? "";
			// Bare /claude-coder (or ui/settings) opens the interactive panel with live previews.
			...
			// Folded former standalone commands: /claude-coder theme …, /claude-coder spinner ….
			...
			if (sub === "group") {
				...
				if (next === undefined) {
					if (ctx.hasUI)
						ctx.ui.notify(
							`Usage: /claude-coder group ${TOOL_BOOL_MODES.join("|")}`,
							"error",
						);
					return;
				}
			...
			if (sub === "branch") {
				...
				if (!Number.isFinite(gray) || gray < 0 || gray > 255) {
					if (ctx.hasUI)
						ctx.ui.notify(
							"Usage: /claude-coder branch <0-255> | theme | fixed | reset",
							"error",
						);
					return;
				}
			...
			if (sub === "diff") {
				...
				if (!Number.isFinite(n) || n <= 0 || n > 150) {
					if (ctx.hasUI)
						ctx.ui.notify(
							"Usage: /claude-coder diff <1-150> | stock | status",
							"error",
						);
					return;
				}
			...
			if (sub === "detail" || sub === "extra") {
				...
				if (next === undefined) {
					if (ctx.hasUI)
						ctx.ui.notify(
							`Usage: /claude-coder detail ${TOOL_BOOL_MODES.join("|")}`,
							"error",
						);
					return;
				}
			...
			if (!(TOOL_MODES as readonly string[]).includes(sub)) {
				if (ctx.hasUI) {
					ctx.ui.notify(
						`Unknown option "${sub}". Try /claude-coder status, /claude-coder branch theme, or /claude-coder group toggle.`,
						"error",
					);
				}
				return;
	...
	// /claude-coder theme — toggle pi-theme-adaptive coloring at runtime.
	// Plain handler object (not a registered command); routed from /claude-coder.
	...
	// /claude-coder spinner — pick which theme color keys drive the spinner verb
	// and status suffix. Plain handler object; routed from /claude-coder.
	...
	const spinnerCommand: Parameters<ExtensionAPI["registerCommand"]>[1] = {
		...
		async handler(args, ctx) {
			...
			if (sub !== "verb" && sub !== "status") {
				if (ctx.hasUI)
					ctx.ui.notify(
						`Usage: /claude-coder spinner verb <key> | status <key> | reset | preview`,
						"error",
					);
				return;
			}
			...
			if (!key) {
				if (ctx.hasUI)
					ctx.ui.notify(
						`Missing color key. Try /claude-coder spinner preview to see available keys.`,
						"error",
					);
				return;
			}
	...
	pi.on("session_start", async (event, ctx) => {
		...
		if (readSettings().ccMyPiSetupDone !== true) {
			...
			// /claude-coder setup re-runs it. The 250ms defer lets theme extensions and
